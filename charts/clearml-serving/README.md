...
|-----|------|---------|-------------|
| clearml | object | `{"apiAccessKey":"ClearML API Access Key","apiHost":"http://clearml-server-apiserver:8008","apiKeysSecretName":"clearml-api-keys","apiSecretKey":"ClearML API Secret Key","createApiKeysSecret":true,"defaultBaseServeUrl":"http://127.0.0.1:8080/serve","filesHost":"http://clearml-server-fileserver:8081","kafkaServeUrl":"","servingTaskId":"ClearML Serving Task ID","webHost":"http://clearml-server-webserver:80"}` | ClearMl generic configurations |
| clearml.apiKeysSecretName | string | `"clearml-api-keys"` | Secret name for api keypair, will be mounted to pods if not empty (if empty - keypair will be set via env vars directly) |
| clearml.createApiKeysSecret | bool | `true` | Condition for creating secret with api keypair (if false - use existing secret) |
